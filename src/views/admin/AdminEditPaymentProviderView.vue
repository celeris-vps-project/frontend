...
import { getPaymentProvider, getProviderTypes, updatePaymentProvider } from '../../api/admin'
...
onMounted(async () => {
  try {
    // Load provider types and the existing provider in parallel
    const [types, provider] = await Promise.all([
      getProviderTypes(),
      getPaymentProvider(providerId),
    ])
    providerTypes.value = types

    // Populate form from existing provider
    providerType.value = provider.type
    name.value = provider.name
    sortOrder.value = provider.sort_order || 0

    // Populate config values from existing config
    const typeInfo = types.find(t => t.type === provider.type)
    if (typeInfo && provider.config) {
      const cfg = {}
      for (const field of typeInfo.fields) {
        if (field.type === 'wallets') {
          // Populate wallets: start with empty defaults, overlay existing
          const wallets = {}
          cryptoNetworks.forEach(n => { wallets[n] = '' })
          if (provider.config.wallets && typeof provider.config.wallets === 'object') {
            for (const [net, addr] of Object.entries(provider.config.wallets)) {
              wallets[net] = addr || ''
            }
          }
          cfg.wallets = wallets
        } else if (field.type === 'bool') {
          cfg[field.key] = !!provider.config[field.key]
        } else {
...
const selectedTypeInfo = computed(() => {
  return providerTypes.value.find(t => t.type === providerType.value)
})
...
async function handleUpdate() {
  ...
  saving.value = true
  try {
    await updatePaymentProvider(providerId, {
  ...
  <AdminLayout>
    <div class="edit-provider-page">
      <header class="page-header">
        ...
        <h1 class="page-title">{{ t('adminPayment.editTitle') }}</h1>
        <p class="page-subtitle">{{ t('adminPayment.editSubtitle') }}</p>
      </header>
      ...
      <!-- Load Error -->
      <div v-else-if="error && !name" class="glass-card error-state">
        <p class="error-text">{{ error }}</p>
        <button class="action-btn secondary-btn" @click="goBack">{{ t('adminPayment.backToProviders') }}</button>
      </div>

      <!-- Success -->
      <div v-else-if="success" class="glass-card success-state">
        <div class="success-icon">✓</div>
        <h2>{{ t('adminPayment.providerUpdated') }}</h2>
        <p>{{ t('adminPayment.providerUpdatedDesc') }}</p>
      ...
      <div v-else class="form-container">
        ...
        <section class="form-section glass-card">
          ...
          <div class="type-display">
            <div
              ...
              :style="{ '--type-color': typeColors[providerType] || '#888' }"
            >
              <span class="type-icon">{{ typeIcons[providerType] || '💰' }}</span>
              <span class="type-name">{{ selectedTypeInfo?.display_name || providerType }}</span>
              <span class="type-code">{{ providerType }}</span>
              <span class="type-lock">🔒</span>
            </div>
            <span class="type-readonly-hint">{{ t('adminPayment.typeReadonly') }}</span>
          </div>
        </section>
        ...
        <section v-if="selectedTypeInfo" class="form-section glass-card">
          ...
          <div v-for="field in selectedTypeInfo.fields" :key="field.key" class="form-group">
            ...
            <!-- notify_url for epay: show current auto-generated value read-only -->
            <template v-else-if="field.key === 'notify_url' && providerType === 'epay'">
              <label class="form-label">{{ field.label }}</label>
              <div class="auto-generated-hint">
                <span class="auto-tag">{{ t('adminPayment.autoGenerated') }}</span>
                <code v-if="configValues[field.key]" class="webhook-url-inline">{{ configValues[field.key] }}</code>
                <span v-else>{{ t('adminPayment.notifyUrlHint') }}</span>
              </div>
              <span class="form-hint">{{ field.help_text }}</span>
            </template>
        ...
        <div class="form-actions">
          ...
          <button
            ...
            @click="handleUpdate"
          >
            {{ saving ? t('adminPayment.updating') : t('adminPayment.updateProvider') }}
          </button>
        </div>
      </div>
    </div>
...
/* Auto-generated hint for notify_url */
.auto-generated-hint {
  display: flex;
  align-items: center;
  gap: 0.5rem;
